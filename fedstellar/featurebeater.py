...
class Featurebeater(threading.Thread, Observable):
    """
    Thread based Featurebeater that sends a beat message to all the neighbors of a node every `HEARTBEAT_PERIOD` seconds.
    ...
    Args:
        nodo_padre (Node): Node that use the Featurebeater.
    """

    def __init__(self, node_name, neighbors, config: Config):
        Observable.__init__(self)
        threading.Thread.__init__(self, name="Featurebeater-" + node_name)
    ...
    def run(self):
        ...
        It happend ``Featurebeater_REFRESH_NEIGHBORS_BY_PERIOD`` per HEARTBEAT_PERIOD
        """
    ...
    def clear_nodes(self):
        ...
        for n in [
            ...
            logging.info(
                "[Featurebeater] Removed {} from the network ".format(n)
            )
            self.__nodes.pop(n)
    ...
    def get_nodes(self, print=False):
        ...
        if print:
            logging.info("[Featurebeater] Nodes Featurebeater: {}".format(node_list))  # All nodes in the network
            logging.info("[Featurebeater] Nodes role: {}".format(self.__nodes_role))  # All nodes in the network
            logging.info("[Featurebeater] Nodes reference basenode: {}".format(self.__neighbors))  # NodeConnections only with the neighbors

        return node_list
    def stop(self):
        """
        Stop the Featurebeater.
        """
